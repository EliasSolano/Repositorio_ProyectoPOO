from __future__ import annotations # Sirve para usar nombres de clases aún no definidas
import json
import os
import re
...
def validar_rut(rut: str) -> bool:
    ...
    if not rut_limpio:
        return False
    # Asume que si tiene la k al final, es válido
    parte_numerica = rut_limpio[:-1] if rut_limpio.endswith(('K', '0')) else rut_limpio
    
    try:
        if int(parte_numerica) > 9999999:
            return True
        else:
            return False
    except ValueError:
...
class SistemaAsistencia:
    ...
    def agregar_estudiante_global(self, nombre: str, rut: str) -> Estudiante: # Método que crea un estudiante
        # Validar que el RUT sea obligatorio y válido
        if not rut:
             raise ValueError("RUT obligatorio.")
        if not validar_rut(rut):
            raise ValueError("RUT inválido.") 
        if rut in self.estudiantes:
            raise ValueError("RUT ya existe")
            
        st = Estudiante(rut, nombre) 
        self.estudiantes[st.rut] = st
        self._guardar_datos()
        return st

    def actualizar_estudiante(self, rut_antiguo: str, nuevo_nombre: str, nuevo_rut: str) -> Estudiante: # Método que modifica los datos del estudiante
        ...
        st = self.estudiantes.get(rut_antiguo)
        if not st:
            raise ValueError("Alumno no encontrado")
        
        # Verificar si el nuevo RUT es diferente y ya existe
        if nuevo_rut != rut_antiguo and nuevo_rut in self.estudiantes:
            raise ValueError("Nuevo RUT ya existe")
        
        # Si el RUT cambia, se elimina la entrada antigua y se crea la nueva
        if nuevo_rut != rut_antiguo:
            del self.estudiantes[rut_antiguo]
            for sess in self.sesiones.values():
                if rut_antiguo in sess.ruts_presentes:
                    sess.ruts_presentes.remove(rut_antiguo)
                    sess.ruts_presentes.append(nuevo_rut)
        
        st.nombre = nuevo_nombre
        st.rut = nuevo_rut
        self.estudiantes[nuevo_rut] = st
        self._guardar_datos()
        return st

    def eliminar_estudiante(self, rut: str): # Método que elimina un estudiante
        if rut not in self.estudiantes:
            raise ValueError("Alumno no encontrado")

        del self.estudiantes[rut]
        for sess in self.sesiones.values():
    ...
    def editar_sesion(self, sesion_id: int, nueva_fecha: Optional[datetime] = None, nuevos_ruts_presentes: Optional[List[str]] = None): # Editar una sesion
        ...
        if nuevos_ruts_presentes is not None: 
            ruts_validos = set(self.estudiantes.keys()) # Obtiene los RUTs de los estudiantes
            sess.ruts_presentes = [rut for rut in nuevos_ruts_presentes if rut in ruts_validos] # Filtra por RUTs válidos
        self._guardar_datos()
    ...
    def marcar_presentes_multiple(self, sesion_id: int, ruts_estudiantes: List[str]):
        sess = self.sesiones.get(sesion_id)
        if not sess:
            raise ValueError("Sesión no encontrada")
            
        ruts_validos = set(self.estudiantes.keys())
        for rut in ruts_estudiantes:
            # Revisa que el RUT exista y que no esté ya presente
            if rut in ruts_validos and rut not in sess.ruts_presentes: 
                sess.ruts_presentes.append(rut)
        self._guardar_datos()


    def porcentaje_asistencia_por_estudiante(self, codigo_curso: str, rut_estudiante: str) -> float: # Calcula el porcentaje de asistencia
        sesiones = [s for s in self.sesiones.values() if s.codigo_curso == codigo_curso]
        if not sesiones:
            return 0.0
        asistido = sum(1 for s in sesiones if rut_estudiante in s.ruts_presentes) 
...
class AppGUI(ctk.CTk):
    ...
    def construir_frame_alumnos(self):
        ...
        self.entrada_rut_alumno = ctk.CTkEntry(campos, placeholder_text="RUT") 
        self.entrada_nombre_alumno.pack(padx=4, pady=4)
        self.entrada_rut_alumno.pack(padx=4, pady=4)
        btns = ctk.CTkFrame(campos)
        btns.pack(pady=6)
        ctk.CTkButton(btns, text="Agregar", command=self.ui_agregar_alumno).pack(side="left", padx=6)
        ...
        # Se necesita un bind para llenar el formulario de alumnos
    ...
    def refrescar_lista_alumnos(self):
        ...
        for rut, st in sorted(self.sistema.estudiantes.items()):
    ...
    def llenar_formulario_alumno(self, event=None):
        ...
        st = self.sistema.estudiantes.get(rut)
        if not st:
            return
    ...
    def ui_agregar_alumno(self):
        ...
        if not nombre:
            ...
            return
        if not rut:
            messagebox.showwarning("Faltan datos", "RUT obligatorio.")
            return
        try:
    ...
    def ui_editar_alumno(self):
        ...
        if not sel:
            ...
            return
        txt = self.listbox_alumnos.get(sel[0])
        rut_antiguo = txt.split(" - ")[0].replace("RUT: ", "").strip() 
        nuevo_nombre = self.entrada_nombre_alumno.get().strip()
        nuevo_rut = self.entrada_rut_alumno.get().strip() # El nuevo identificador
        ...
        try:
            self.sistema.actualizar_estudiante(rut_antiguo, nuevo_nombre, nuevo_rut) 
    ...
    def ui_eliminar_alumno(self):
        ...
        if messagebox.askyesno("Confirmar", f"Eliminar alumno con RUT {rut}? Esto lo quita de todas las sesiones."):
            try:
                self.sistema.eliminar_estudiante(rut)
    ...
    def ui_editar_presentes_sesion(self):
        ...
        for rut, st in sorted(self.sistema.estudiantes.items()): 
            ...
            cb = tk.Checkbutton(frame, text=f"RUT: {rut} - {st.nombre}", variable=var) # Muestra RUT
            cb.pack(anchor="w")
            var_map[rut] = var
        ...
        def aplicar_cambios():
            ...
            try:
                self.sistema.editar_sesion(sess_id, nuevos_ruts_presentes=nuevos_ruts_presentes) 
    ...
    def refrescar_lista_porcentajes(self, val):
        ...
        if not codigo_curso:
            return
        for rut, st in sorted(self.sistema.estudiantes.items()): 
            pct = self.sistema.porcentaje_asistencia_por_estudiante(codigo_curso, rut) 
